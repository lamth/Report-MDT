...
```
- Tạo user 'root'.'%' để có thể truy cập từ xa:
```
MariaDB [wordpress]>  CREATE USER 'root'@'%' IDENTIFIED BY '123';
#gán tất cả các quyền cho user này.
MariaDB [mysql]> GRANT ALL ON *.* TO root@'%' WITH GRANT OPTION;
